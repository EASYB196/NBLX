import jogger from '../assets/images/MEN/Pants/jogger.png';
import jogger2 from '../assets/images/MEN/Pants/joggerback.png';

import elegantcargojean from '../assets/images/MEN/Jeans/white_cargopant_frontv.png';
import elegantcargojeanbv from '../assets/images/MEN/Jeans/white_cargopant_sidev.png';

import Flameblackjackettrouserfv from '../assets/images/WOMEN/Pant/Flameblackjackettrouserfv.jpg';
import Flameblackjackettrouserbv from '../assets/images/WOMEN/Pant/FlameblackjacketTrouserbv.jpg';

export const PantsDatas = [
  {
    id: 'joggers',
    name: 'Jogger',
    price: 35000,
    oldPrice: 1543000,
    inStock: 4,

    sizes: ['S', 'M', 'L', 'XL', 'XXL', '3XL'],

    // colors: [
    //   {
    //     name: 'Black',
    //     value: '#000000',
    //     images: [jogger, jogger2],
    //   },
    // ],

    image: jogger,
    hoverImage: jogger2,

    description:
      'A clean and comfortable jogger designed for everyday wear with a relaxed, modern fit.',

    features: [
      'Relaxed fit',
      'Comfortable everyday construction',
      'Clean minimalist design',
      'Suitable for casual styling',
    ],

    fabric: 'Premium cotton blend',

    care: [
      'Machine wash cold',
      'Wash with similar colors',
      'Do not bleach',
      'Air dry recommended',
      'Iron on low heat if necessary',
    ],
  },

  {
    id: 'elegant cargo jean',
    name: 'Elegant Cargo Jean',
    price: 50000,
    oldPrice: 1000000,
    inStock: 7,

    sizes: ['S', 'M', 'L', 'XL', 'XXL', '3XL'],

    // colors: [
    //   {
    //     name: 'White',
    //     value: '#FFFFFF',
    //     images: [elegantcargojean, elegantcargojeanbv],
    //   },
    // ],

    image: elegantcargojean,
    hoverImage: elegantcargojeanbv,

    description:
      'A modern cargo jean featuring a clean silhouette and functional detailing for a versatile everyday look.',

    features: [
      'Modern cargo silhouette',
      'Functional cargo detailing',
      'Comfortable everyday fit',
      'Versatile styling',
    ],

    fabric: 'Premium denim',

    care: [
      'Machine wash cold',
      'Turn inside out before washing',
      'Wash with similar colors',
      'Do not bleach',
      'Air dry recommended',
    ],
  },

  // {
  //   id: 'flare-flame-trouser',
  //   name: 'Flare Flame Trouser',
  //   price: 1273800,
  //   oldPrice: 1543000,
  //   inStock: 4,

  //   sizes: ['S', 'M', 'L', 'XL', 'XXL', '3XS'],

  //   colors: [
  //     // {
  //     //   name: 'Black',
  //     //   value: '#000000',
  //     //   images: [
  //     //     Flameblackjackettrouserfv,
  //     //     Flameblackjackettrouserbv,
  //     //   ],
  //     // },
  //   ],

  //   image: Flameblackjackettrouserfv,
  //   hoverImage: Flameblackjackettrouserbv,

  //   description:
  //     'A statement trouser designed with a distinctive flare silhouette.',

  //   features: [
  //     'Flared silhouette',
  //     'Statement design',
  //     'Modern fit',
  //   ],

  //   fabric: 'Premium fabric',

  //   care: [
  //     'Machine wash cold',
  //     'Wash with similar colors',
  //     'Do not bleach',
  //     'Air dry recommended',
  //   ],
  // },
];
