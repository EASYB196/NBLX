...
import { KaftanData } from '../../../data/KafatanData';

const KaftanDatas = ({ product }) => {
  const [isHovered, setIsHovered] = useState(false);

  return (
    <div className='md:px-5 font-serif'>
      <Link to={`/kafans-shirts/${product.id}`}>
        <div
        ...
        >
          ...
          <h3 className='text-sm font-semibold  text-black px-1 bg-red-400 rounded-xl w-19 text-center relative bottom-118 left-2 ml-3 md:ml-0'>ONSALE</h3>
      ...
      <div className=" bg-white text-black ml-4 md:ml-0">
        ...
        <p className="text-black font-[cinzel] font-bold">₦{product.price.toLocaleString('en-NG')}</p>
  ...
  return (
    <div className='bg-white mt-23 z-10 pt-10'>
      <div className='space-y-7'>
        <h1 className='text-black text-5xl text-center '>Kaftan</h1>

        <div className='flex items-center justify-center gap-5'>
          <Link to={'/'} className='text-black'>Home</Link> <FaChevronRight className='text-black w-2' /> <span className='text-black'>Kaftan/shirts</span>
        </div>
      </div>

      <div className="grid grid-col md:grid-cols-4 gap-5 p-4 mt-25">
        {KaftanData.map((product) => (
          <KaftanDatas key={product.id} product={product} />
        ))}
      </div>
